...
class AsyncCryptoStore {
  ...
  async getEndToEndSessions(deviceKey, txn, func) {
    ...
    for (const k of allKeys.filter((k) =>
      k.startsWith(E2E_PREFIX + "sessions/" + deviceKey + "/")
    )) {
  ...
  async addEndToEndInboundGroupSession(
    senderCurve25519Key,
    sessionId,
    sessionData,
    txn
  ...
  async markSessionsNeedingBackup(sessions) {
    const sessionsNeedingBackup =
      this._getJsonItem(KEY_SESSIONS_NEEDING_BACKUP) || {};
    for (const session of sessions) {
   ...
   */
  async deleteAllData() {
    const allE2eStorageKeys = (await this.asyncStorage.getAllKeys()).filter(
      (k) => k.startsWith(E2E_PREFIX)
    );
